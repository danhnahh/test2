		...
		else
		{
			...
			while (!quit)
			{
				...
				while (SDL_PollEvent(&e) != 0)
				{
					...
					if (start == 1) {
						if (e.type == SDL_MOUSEBUTTONDOWN && x >= 64 && x <= 343 && y >= 239 && y <= 431) 
						{
							typegame = 1;
							start = 2;
						}
						/*if (e.type == SDL_MOUSEBUTTONDOWN && x >= 487 && x <= 766 && y >= 243 && y <= 421)
						{
							typegame = 2;
							start = 2;
						}*/
				...
				if (start == 2) {
					if (typegame == 1)
					{
						if (pause == false)
						{
							...
							/*float X = x;
							float Y = y;
							float delta_x = X - bullet.x;
							float delta_y = Y - bullet.y;
							...
							if (dem >= 5)
							{
								...
								if (thubossquaydau == true) {
									...
									SDL_Rect banbullet;
									banbullet = { bullet.x += 5 * cos(angle), bullet.y += 5 * sin(angle), 30,30};
